...
const self = {
  ...
  created: () => {
    ...
    new AddComputer(200, 2200, 0, 'player', self.friendCount)
  },
  update: () => {
  },
  functions: [
    () => {
      new AddComputer(3200, 2000, 180, 'enemy', 3)
      new AddComputer(5000, 3000, 190, 'enemy', 3)
      new AddComputer(7400, 1600, 170, 'enemy', 3)
      return true
    },
    ...
    () => {
      new AddComputer(2000, 1000, 100, 'enemy', 2)
      new AddComputer(2000, 5000, 260, 'enemy', 4)
      return true
    },
    () => !state.field.enemy.children.length,
    () => {
    ...
    () => {
      if (state.field.is('sublatant_4')) {
        self.route = 1
        new AddComputer(2000, 1600, 180, 'enemy', 2)
        return true
      }
      if (state.field.is('sublatant_5')) {
        self.route = 2
        new AddComputer(2400, 900, 130, 'enemy', 3)
        new AddComputer(2400, 3500, 230, 'enemy', 4)
        return true
      }
    },
    () => !state.field.enemy.children.length,
    () => {
      if (self.route === 1) {
        new AddComputer(6500, 1600, 180, 'enemy', 3)
        return true
      }
      if (self.route === 2) {
        new AddComputer(6000, 1000, 150, 'enemy', 5)
        new AddComputer(6000, 3000, 210, 'enemy', 4)
        return true
      }
    },
    () => !state.field.enemy.children.length,
    ...
    () => {
      ...
      if (state.field.is('sublatant_6')) {
        new AddComputer(1500, 2400, 260, 'enemy', 2)
        new AddComputer(2500, 2800, 270, 'enemy', 2)
        return true
      }
    },
    () => {
      ...
      if (!state.field.enemy.children.length) {
        new AddComputer(900, 6800, 260, 'enemy', 2)
        new AddComputer(3200, 6200, 270, 'enemy', 2)
        return true
      }
    },
    () => {
