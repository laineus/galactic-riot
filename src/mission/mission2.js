...
const self = {
  ...
  created: () => {
    ...
    new new AddComputer(1000, -200, 280, 'player', self.friendCount)
  },
  update: () => {
  },
  functions: [
    () => {
      new AddComputer(1600, -600, 270, 'enemy', 2)
      return true
    },
    () => !state.field.enemy.children.length,
    () => {
      new AddComputer(1200, 3700, 260, 'enemy', 3)
      return true
    },
    () => !state.field.enemy.children.length,
    ...
    () => {
      new AddComputer(1100, 1700, 340, 'enemy', 2)
      return true
    },
    () => !state.field.enemy.children.length,
    () => {
      new AddComputer(2500, 1400, 10, 'enemy', 3)
      return true
    },
    () => !state.field.enemy.children.length
  ]
