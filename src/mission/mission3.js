...
const self = {
  ...
  created: () => {
    ...
    new AddComputer(200, 3200, 350, 'player', self.friendCount)
  },
  update: () => {
  },
  functions: [
    () => {
      new AddComputer(-200, 200, 165, 'enemy', 2)
      new AddComputer(-200, 1600, 170, 'enemy', 3)
      return true
    },
    () => !state.field.enemy.children.length,
    () => {
      new AddComputer(200, -200, 345, 'enemy', 3)
      new AddComputer(200, 200, 15, 'enemy', 4)
      return true
    },
    () => !state.field.enemy.children.length,
    ...
    () => {
      new AddComputer(-200, -200, 225, 'enemy', 5)
      return true
    },
    () => !state.field.enemy.children.length
  ]
